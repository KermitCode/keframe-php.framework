...
return array(
	...
	#自动加载的扩展类
	'extendClass'=>array(
		#'a'=>APPPATH.'dirname'				#如还需要加载类，将路径添加至此
	),
    ...
    'cache' => array(
        ...
        'cache_path' => APPPATH.'cache',    #为空则默认放在对应系统或子系统的cache目录下/，如固定在application目录下可使用 APPPATH.cache
    ...
    'log' => array(
		...
		'slowdbquery'=>500,				#记录SQL查询耗时多少ms以上的查询日志,单位毫秒
		'slowinterquery'=>500,				#记录接口调用耗时多少ms以上的查询日志,单位毫秒
    ),

	#URL伪静态配置
	'url_rewrite'=>array(

		'rewrite_open'=>true,				/*重写规则：false不重写，将使用index.php?ca=控制器/方法&参数的方法
													    true重写，将使用 /[application/]conroller/action/参数名/值...重写为?_ke=前面的uri;*/
