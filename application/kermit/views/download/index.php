					...
					<table class="table table-bordered table-striped">
						...
						$i=1;foreach($this->downData as $k=>$row){?>
							<tr>
								...
								<td><a href="<?php echo $this->makeUrl('download/view',array('id'=>$row['id']));?>" target="_blank"><?php echo $row['title'];?></a>
                                <br /><?php echo $row['filepath'];?></td>
                                <td><?php echo date('m-d H:i:s',$row['addtime']);?></td>
                                <td><?php echo $this->baseUrl."download/{$row['id']}.html";?></td>
                                <td><?php echo $row['downloads'];?></td>
                                <td><?php 
									echo "<a href='".$this->makeUrl('download/add',array('id'=>$row['id']))
				."'><button class='btn btn-success btn-small' type='button' >修改</button></a>&nbsp;&nbsp;";
									echo "<a href='".$this->makeUrl('download/delete',array('id'=>$row['id']))
